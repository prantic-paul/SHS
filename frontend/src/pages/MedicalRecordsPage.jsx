...
const MedicalRecordsPage = () => {
  ...
  const getFilteredPrescriptions = () => {
    ...
    switch (filterBy) {
      case 'recent':
        return prescriptions.filter(p => new Date(p.created_at) >= thirtyDaysAgo);
      case 'older':
        return prescriptions.filter(p => new Date(p.created_at) < thirtyDaysAgo);
      default:
          ...
          {filteredPrescriptions.length === 0 ? (
            <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
              ...
              <h3 className="text-xl font-semibold text-gray-900 mb-2">
                {filterBy === 'all' ? 'No Medical Records' : `No ${filterBy} records found`}
              </h3>
              <p className="text-gray-600 mb-6">
                {filterBy === 'all' 
                  ? 'Your prescription history will appear here after your first appointment.'
                  : 'Try changing the filter to view other records.'}
              </p>
              <button
